...
void buscaPalabras(char palabras[][10],int *bandera,char cadena[],int *contPalabrasDist);
void five_pals ();
void seven_pals ();
void nine_pals ();
...
void five_pals(){
    ...
    while(!feof(arch)){
        fscanf(arch,"%5s",n); //leo hasta 5 digitos
         buscaPalabras(palabras,&bandera,n,&contPalabrasDist); //busca la palabra, si la encuentra devuelve la posicion donde estaba para aumentar la frecuencia, si no la encuentra, la agrega y hace lo mismo
