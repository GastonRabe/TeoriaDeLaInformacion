...
void getEntropiaInfo(int cantPalabrasDist,float *entropia,float *informacion,float probPalabras[],float frecuenciaPalabras[],float totalPalabras,char palabras[][10],FILE *archEscritura);
...
void procesoPalabras(int nroDigitos,Tlista *l,FILE *arch);
...
int main()
{
    ...
    FILE *arch5_2,*arch7_2,*arch9_2,*arch5,*arch7,*arch9;
    FILE *arch5_3,*arch7_3,*arch9_3;

    arch5=fopen("1A 2B 2C Escenario 1.txt","w");
    procesoPalabras(5,&l,arch5);
    arch5_3=fopen("Codificación Huffman 5.txt","w");
    huffman(l->a,"",5,arch5_3);
    arch5_2=fopen("Reconstrucción Huffman 5.txt","w");
    compresionHuffman(l,5,arch5_2);
    fclose(arch5);fclose(arch5_2);fclose(arch5_3);
    free(l);

    printf("\n\n-----------------------------------------\n");
    printf("-----------------------------------------\n\n\n");

    arch7=fopen("1A 2B 2C Escenario 2.txt","w");
    procesoPalabras(7,&l,arch7);
    arch7_3=fopen("Codificación Huffman 7.txt","w");
    huffman(l->a,"",7,arch7_3);
    arch7_2=fopen("Reconstrucción Huffman 7.txt","w");
    compresionHuffman(l,7,arch7_2);
    fclose(arch7);fclose(arch7_2);fclose(arch7_3);
    free(l);

    printf("\n\n-----------------------------------------\n");
    printf("-----------------------------------------\n\n\n");

    arch9=fopen("1A 2B 2C Escenario 3.txt","w");
    procesoPalabras(9,&l,arch9);
    arch9_3=fopen("Codificación Huffman 9.txt","w");
    huffman(l->a,"",9,arch9_3);
    arch9_2=fopen("Reconstrucción Huffman 9.txt","w");
    compresionHuffman(l,9,arch9_2);
    fclose(arch9);fclose(arch9_2);fclose(arch9_3);
    free(l);
    return 0;
}
  ...
  char auxBinario[20]="";
    if(a){
        ...
        if(strlen(a->simbolo)==longitud){
            ...
            fprintf(arch,"Símbolo: %s / Código Huffman: %s\n",a->simbolo,a->codigoBinario);
        }
...
void getEntropiaInfo(int cantPalabrasDist,float *entropia,float *informacion,float probPalabras[],float frecuenciaPalabras[],float totalPalabras,char palabras[][10],FILE *archEscritura){
    ...
    for(i=0;i<cantPalabrasDist;i++){
        ...
        fprintf(archEscritura,"Símbolo: %s / Cantidad de información del símbolo: %4.2f\n",palabras[i],auxInformacion);
...
void procesoPalabras(int nroDigitos,Tlista *l,FILE *archEscritura){
    ...
    *l=NULL;
    ...
    fprintf(archEscritura,"Fuente con palabras de %d dígitos:\n",nroDigitos);
    getEntropiaInfo(cantPalabrasDist,&entropia,&informacion,probPalabras,frecuenciaPalabras,totalPalabras,palabras,archEscritura);
    kraft=kraftMcMillan(palabras,cantPalabrasDist);
    if(kraft<=1.0){
        printf("Cumple con la inecuación de Kraft/McMillan: %2.1f\n",kraft);
        fprintf(archEscritura,"Cumple con la inecuación de Kraft/McMillan: %2.1f\n",kraft);
    }
    else{
        printf("No cumple con la inecuación de Kraft/McMillan: %2.1f >= 1\n",kraft);
        fprintf(archEscritura,"No cumple con la inecuación de Kraft/McMillan: %2.1f >= 1\n",kraft);
    }
    printf("Cantidad de información total de la fuente: %5.1f (en bits)\n",informacion);
    fprintf(archEscritura,"Cantidad de información total de la fuente: %5.1f (en bits)\n",informacion);
    printf("Entropía total de la fuente: %5.1f (en bits)\n",entropia);
    fprintf(archEscritura,"Entropía total de la fuente: %5.1f (en bits)\n",entropia);
    longitudMedia(&rtaLongMedia,probPalabras,nroDigitos,cantPalabrasDist);
    printf("Longitud media del código: %4.2f\n",rtaLongMedia);
    fprintf(archEscritura,"Longitud media del código: %4.2f\n",rtaLongMedia);
    if(entropia <= rtaLongMedia){
        printf("El código es candidato a ser compacto\n");
        fprintf(archEscritura,"El código es candidato a ser compacto\n");
    }
    else{
        printf("El código no es candidato a ser compacto\n");
        fprintf(archEscritura,"El código no es candidato a ser compacto\n");
    }
    ...
    fprintf(archEscritura,"Rendimiento del código: %f / Redundancia del código: %f\n",rendimiento,redundancia);
    printf("---------------------------------------------------------------\n");
    fclose(arch);
