...
void muestraMat(float mat[][4],char simbolos[][3],FILE *archSalida);
...
void muestraVector(float vec[],FILE *archSalida);
...
int main()
{
    ...
    FILE *archSalida;
    archSalida=fopen("Resultados 1B 1C.txt","w");
    ...
    for (i=0;i<4;i++){
        ...
        fprintf(archSalida,"Probabilidad palabra %s: %5.3f\n",simbolos[i],probabilidades[i]);
    }
    fclose(arch);
    divideMatriz(matrizCondicional,frecuencia);
    printf("\n");
    muestraMat(matrizCondicional,simbolos,archSalida);
    printf("\n");
    if(ergodica(matrizCondicional)){
        printf("Es una fuente markoviana ergódica \n");
        copiaMatriz(matrizCondicional,matrizCondicionalAux);
        printf("Ingrese pasos de iteración para el vector estacionario\n");
        scanf("%d",&paso);
        vectorEstacionario(matrizCondicional,paso,vecEstacionario);
        muestraVector(vecEstacionario,archSalida);
        calculaEntropia(matrizCondicionalAux,&entropia,vecEstacionario);
        printf("Entropía total de la fuente: %5.4f bits\n",entropia);
        fprintf(archSalida,"Entropía total de la fuente: %5.4f bits\n",entropia);
    }
    fclose(archSalida);
    return 0;
}
...
void muestraVector(float vec[],FILE *archSalida){
    ...
    fprintf(archSalida,"Vector Estacionario V*: |");
    for(i=0;i<4;i++){
        printf("%5.4f|",vec[i]);
        fprintf(archSalida,"%5.4f|",vec[i]);
    }
    printf("\n");
     fprintf(archSalida,"\n");
...
void muestraMat(float mat[][4],char simbolos[][3],FILE *archSalida){
    int i,j;
    float sumaCol;
    printf("Matriz de Transición:\n");
    fprintf(archSalida,"Matriz de Transición:\n");
    printf("          00      01      10      11\n");
    fprintf(archSalida,"          00      01      10      11\n");
    for(i=0;i<4;i++){
        printf("%s\t",simbolos[i]);
        fprintf(archSalida,"%s\t",simbolos[i]);
        for(j=0;j<4;j++){
          printf("%5.4f\t",mat[i][j]);
          fprintf(archSalida,"%5.4f\t",mat[i][j]);
        }
        printf("\n");
        fprintf(archSalida,"\n");
    }
    ...
    for(j=0;j<4;j++){
        ...
        for(i=0;i<4;i++)
            sumaCol+=mat[i][j];
        printf("%4.3f|",sumaCol);
        fprintf(archSalida,"%4.3f|",sumaCol);
    }
    printf("\n");
    fprintf(archSalida,"\n");
